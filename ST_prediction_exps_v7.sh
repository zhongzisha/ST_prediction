...
echo "begin training"
for VAL_INDEX in 0; do
torchrun \
    --nnodes=1 \
    --nproc_per_node=${NUM_GPUS} \
    --rdzv_backend=c10d \
    --rdzv_endpoint=localhost:${PORT} \
    ST_prediction_exps_v7.py \
    --action train \
    --num_gpus ${NUM_GPUS} \
    --data_root ${DATA_ROOT} \
    --backbone ${BACKBONE} \
...
python ST_prediction_exps_v7.py \
  --action test \
  --num_gpus ${NUM_GPUS} \
  --data_root ${DATA_ROOT} \
  --backbone ${BACKBONE} \
...
if [ $ACTION == "plot" ]; then
cd $current_dir;
srun python ST_prediction_exps_v7.py \
  --action plot \
  --num_gpus ${NUM_GPUS} \
  --data_root ${DATA_ROOT} \
  --backbone ${BACKBONE} \
...
ST_prediction_exps_v7.sh ${NUM_GPUS} ${BACKBONE} ${LR} ${BS} ${USE_SMOOTH} ${FIX_BACKBONE} ${DATA_ROOT} ${ACTION}
done
done
done
done
